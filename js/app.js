...
/* FUNCTION FOR FOOTER PROJECTS TRIGGER */

document.addEventListener("DOMContentLoaded", () => {
  const footerProjectsTrigger = document.getElementById("openProjectsMenu");
  const hamburgerTrigger = document.querySelector(".mxd-nav__hamburger");

  if (footerProjectsTrigger && hamburgerTrigger) {
    footerProjectsTrigger.addEventListener("click", (e) => {
      e.preventDefault();

      if (!hamburgerTrigger.classList.contains("nav-open")) {
...
document.addEventListener("DOMContentLoaded", () => {
  const skillRows = document.querySelectorAll(".awSkillRow");

  skillRows.forEach((row) => {
    const level = row.getAttribute("data-level");
    const fill = row.querySelector(".awSkillRow__fill");
