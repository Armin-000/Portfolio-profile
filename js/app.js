...
(() => {
  ...
  let START_Y_FRAC = 0.25;
  let WAVE_CYCLES = 5.5;
  let BEND_STRENGTH = 1.50;
  let DENSITY = 0.80;
  let SPEED_MULT = 0.45;
  let TARGET_FPS = 60;
  let SCATTER_START = 0.95;
  let SCATTER_AMOUNT = 1.00;
  let STREAMS_MULT = 3.5;
  let WAVE_STRENGTH = 0.40;

  const BASE_COLS_PER_SEC = 5;
  const BASE_PHASE_SPEED = 1.0;
  ...
  function localizedInfluence(p, center, radius) {
    ...
    const norm = Math.min(1, dist / Math.max(1e-6, radius));
  ...
  function applyBends(p, yCurrent, h) {
    let y = yCurrent;
    y += globalWaveOffset(p, h);
  ...
  function effectiveSpeed() {
    ...
    const gamma = 0.8;
    return MIN_BOOST + Math.pow(t, gamma) * (MAX_BOOST - MIN_BOOST);
  }

  function fadeTransparent() {
  ...
  const DESKTOP_BAND_TOP = TARGET_BAND_TOP_FRAC;
  const DESKTOP_START_Y = START_Y_FRAC;
  const DESKTOP_WAVE_CYCLES = WAVE_CYCLES;
  const DESKTOP_WAVE_STRENGTH = WAVE_STRENGTH;
  const DESKTOP_SPEED_MULT = SPEED_MULT;
  const DESKTOP_BEND_STRENGTH = BEND_STRENGTH;

  function applyResponsive() {
    if (window.matchMedia("(max-width: 1199px)").matches) {
      TARGET_BAND_TOP_FRAC = 0.05;
      START_Y_FRAC = 0.30;
      WAVE_CYCLES = DESKTOP_WAVE_CYCLES * 0.0;
      ...
      WAVE_CYCLES = DESKTOP_WAVE_CYCLES;
  ...
  function resizeCanvas() {
    ...
    const baseFont = Math.max(12, Math.floor(viewW / 120));
    dynamicFontSize = Math.max(8, Math.floor(baseFont / DENSITY));

    bandTopPx = Math.floor(viewH * TARGET_BAND_TOP_FRAC);
    const bandTop = bandTopPx;
    const bandHeight = Math.floor(viewH * TARGET_BAND_HEIGHT_FRAC);
    const bandBottom = Math.min(viewH - 2, bandTop + bandHeight);
    ...
    const fullCols = Math.max(1, Math.floor(viewW / dynamicFontSize));
    h_drops = [];
    ...
    for (let i = 0; i < rows; i++) {
      ...
      const scatterY = Math.max(dynamicFontSize, Math.min(viewH - 2, Math.random() * viewH));
  ...
  function drawStep(dt) {
    ...
    const diagonalRisePx = viewH * DIAGONAL_RISE_RATIO;
    const slopePerCol = diagonalRisePx / Math.max(1, fullCols);

    const margin = Math.max(dynamicFontSize, viewH * 0.03);
    let startY = Math.round(clamp01(START_Y_FRAC) * viewH);
    startY = Math.max(margin, Math.min(viewH - margin, startY));

    const effSpeed = effectiveSpeed();
    const waveAmplitude = viewH * 0.00;
    ...
    for (let i = 0; i < h_drops.length; i++) {
      ...
      yDiag = applyBends(p, yDiag, viewH);
      ...
      const yDraw = Math.max(dynamicFontSize, Math.min(viewH - 2, wavyY));
      ...
      if (drop.x > drop.resetPoint && Math.random() > 0.95) {
        ...
        drop.lastY = null;
        drop.scatterY = Math.max(dynamicFontSize, Math.min(viewH - 2, Math.random() * viewH));
      }
    }
  }
