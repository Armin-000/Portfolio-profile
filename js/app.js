...
// --------------------------------------------- //
// Dark Only (no switch, no OS detection)
// --------------------------------------------- //
window.addEventListener('DOMContentLoaded', () => {
  ...
  try { localStorage.removeItem('template.theme'); } catch (e) {}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Fallback: never leave cards invisible
  ...
  // Per-card reveal + fill (no "all at once")
  ...
  const cardIO = new IntersectionObserver((entries) => {
    entries.forEach((e) => {
      ...
      if (e.isIntersecting) {
        ...
        // fill only when entering (and re-fill when returning)
        animateTo(card, card._aw.level, 950);
        seen.add(card);
      } else {
        // empty when leaving viewport (if you want that behavior)
  ...
  // Ensure cards aren't stuck hidden
  // If you keep CSS .is-ready hiding cards, we must set initial hidden per card here:
  cards.forEach((card) => {
    // initial state (nice, but not "gone" forever)
  ...
  // If after load nothing triggered (rare), show all
  setTimeout(() => {
  ...
  (function () {
    ...
    // 1) Hamburger opener (tvoj existing)
    const burger = document.querySelector(".mxd-nav__hamburger");

    // 2) "My projects" toggle u main meniju (span.btn-anim koji otvara submenu)
    // U tvom meniju to je element: .main-menu__item -> .main-menu__toggle -> span.main-menu__link.btn.btn-anim
    const projectsToggle = Array.from(document.querySelectorAll(".main-menu__link.btn.btn-anim"))
      .find(el => (el.textContent || "").trim().toLowerCase() === "my projects");

    const openHamburger = () => {
      if (!burger) return;
      burger.click(); // oslanja se na tvoj postojeći app.js toggle
    };

    const openProjectsSubmenu = () => {
      if (!projectsToggle) return;

      // Klikni "My projects" u overlay meniju (trebao bi otvoriti submenu)
      projectsToggle.click();

      // Po želji: scroll do submenu-a da bude u fokusu
    ...
    btn.addEventListener("click", (e) => {
      ...
      // Otvori hamburger menu
      openHamburger();

      // Pričekaj tick da se overlay prikaže pa otvori submenu
